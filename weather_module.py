import requests
import datetime  # модуль запрашивающий дату и время

appid = "6a8d9e01be3b7c56f1b5eb58affba33f"  # наш API ключ


def get_weather(place, time=0):  # функция, запрашивающая погоду у API
    try:
        ...
        if time == 0:
            ...
            for prediction in data['list']:
                if prediction['dt_txt'] == '%s 15:00:00' % (date):
                    ...
                    return prediction['weather'][0]['description'] + ', ' + clouds + ', температура ' + str(
